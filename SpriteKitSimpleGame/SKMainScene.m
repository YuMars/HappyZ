...
-(instancetype)initWithSize:(CGSize)size {
    ...
    if (self) {
        ...
        self.isGravity = YES;
        self.sensitivity = 3000;
        
        if (@available(iOS 11.0, *)) {
            self.currentPoint = CGPointMake(size.width / 2.0, 0 + UIApplication.sharedApplication.windows.lastObject.safeAreaInsets.bottom + _playerPlane.size.height);
        } else {
            self.currentPoint = CGPointMake(size.width / 2.0, size.height / 2.0 - _playerPlane.size.height);
        }
...
//    return;
    ...
    if (_smallPlaneTime > 75)
    {
        float speed = (arc4random() % 5) + 4;
...
//    if (self.lastUpdateTime) {
        ...
        //        计算两次更新之间有多长时间
        NSTimeInterval updatePeriod = [[NSDate date] timeIntervalSinceDate:self.lastUpdateTime];
    ...
    self.currentPoint = CGPointMake(self.currentPoint.x + self.ballXVelocity * updatePeriod * self.sensitivity, self.currentPoint.y);
        
//        NSLog(@"currentPoint.x = %f",self.currentPoint.x);
//        NSLog(@"self.accelleration x:%f y:%f z:%f", self.accelleration.x, self.accelleration.y, self.accelleration.z);
//    }
...
- (void)setCurrentPoint:(CGPoint)currentPoint {
    ...
    if (_currentPoint.x <= _playerPlane.size.width / 2) {
        _currentPoint.x = _playerPlane.size.width / 2;
        self.ballXVelocity = 0.0;
    }
    
    //    判断球是否在X轴方向碰触到了右侧边缘。如果碰触到了，不让其出界，同时将X方向的加速度反向减半
    if (_currentPoint.x >= self.frame.size.width - _playerPlane.size.width / 2 ) {
        _currentPoint.x = self.frame.size.width - _playerPlane.size.width / 2 ;
        self.ballXVelocity = 0.0;
    }
    
    //    重新设置imageView的位置
//    _playerPlane.center = _currentPoint;
    
    SKAction *action = [SKAction moveTo:CGPointMake(_currentPoint.x, _currentPoint.y) duration:0.1];
    [_playerPlane runAction:action];
}
